...
${
    ...
    Template(Settings settings)
    {
        ...
        settings.IncludeProject("OpenHumanTask.Runtime.Domain");
    ...
    string GetOutputFilename(string templateType, File file)
    {
        ...
        string relativeDirectoryPath = System.IO.Path.Combine("Models", "Generated");
        string absoluteDirectoryPath = System.IO.Path.Combine(srcDirectory.FullName, "core", "OpenHumanTask.Runtime.Integration", relativeDirectoryPath);
    ...
    string GetBaseClass(Class c) 
    {
        ...
        else if (c.BaseClass.Name == "Entity")
            return "Entity";
        else
...
");
    System.IO.DirectoryInfo aggregateDirectory = new System.IO.FileInfo(((File)c.Parent).FullName).Directory;
        output.AppendLine($"namespace OpenHumanTask.Runtime.Integration.Models");
        output.AppendLine("{");
    ...
    string PropertyDeclarations(Class c)
    {
        ...
        foreach(Property property in c.Properties.Where(p => !p.Attributes.Any(a => a.Name == "ProjectNever") || p.Attributes.Any(a => a.Name == "Map")))
        {
            ...
            Attribute jsonPropertyAttribute = property.Attributes.FirstOrDefault(a => a.Name == "JsonPropertyName");
            ...
            output.AppendLine(Indent(2, $"[DataMember(Name = \"{jsonPropertyName}\", Order = {order})]"));
            output.AppendLine(Indent(2, $"[JsonPropertyName(\"{jsonPropertyName}\"){( propertyType == "object" || propertyType == "object?" ? ", JsonConverter(typeof(JsonElementConverter))" : null)}]"));
            output.AppendLine(Indent(2, $"[YamlMember(Alias = \"{jsonPropertyName}\")]"));
            if(property.DocComment != null)
            ...
            output.AppendLine(Indent(2, $"public virtual {propertyType} {property.Name} {{ get; set; }}"));
            order++;
        }
        return output.ToString();
    }

}$Classes(OpenHumanTask.Runtime.Domain.Models.*)[$NamespaceDeclaration
