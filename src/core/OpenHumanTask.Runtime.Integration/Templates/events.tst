...
${
    ...
    string GetOutputFilename(string templateType, File file)
    {
            ...
            && c.Name.EndsWith("DomainEvent"))
        {
            ...
            string directoryName = $"Events\\{aggregateDirectory.Name}\\Generated";
...
");
    System.IO.DirectoryInfo aggregateDirectory = new System.IO.FileInfo(((File)c.Parent).FullName).Directory;
      output.AppendLine($"namespace OpenHumanTask.Runtime.Integration.Events.{aggregateDirectory.Name}");
        output.AppendLine("{");
...
}$Classes(OpenHumanTask.Runtime.Domain.Events.*)[$NamespaceDeclaration
