...
namespace OpenHumanTask.Runtime.UnitTests.Cases.Domain.Aggregates
{
    ...
    public class HumanTaskTests
    {
        ...
        [Fact]
        public void Complete_Should_Work()
        {
            //arrange
            var task = HumanTaskFactory.Create();
            var user = task.PeopleAssignments.PotentialOwners!.First();
            var outcome = new { en = "fake-output-en", fr = "fake-output-fr" };
            var output = new { fakeProperty = "fake-data" };
            var completionBehaviorName = "fake-completion-behavior";
            task.Claim(user);
            task.Start(user);
            task.ClearPendingEvents();

            //act
            task.Complete(user, outcome, output, completionBehaviorName);

            //assert
            task.PendingEvents.Should().NotBeNullOrEmpty();
            task.PendingEvents.Should().HaveCount(2);
            task.PendingEvents.OfType<HumanTaskCompletedDomainEvent>().Should().ContainSingle();
            task.PendingEvents.OfType<HumanTaskStatusChangedDomainEvent>().Should().ContainSingle();
            task.Status.Should().Be(HumanTaskStatus.Completed);
            task.CompletedAt.Should().NotBeNull();
            task.Outcome.Should().BeEquivalentTo(outcome);
            task.Output.Should().BeEquivalentTo(output);
            task.CompletionBehaviorName.Should().Be(completionBehaviorName);
        }

        [Fact]
        public void Complete_With_Null_User_Should_Throw()
        {
            //arrange
            var task = HumanTaskFactory.Create();
            var user = task.PeopleAssignments.PotentialOwners!.First();
            ...
            task.Start(user);
            task.ClearPendingEvents();
            var action = () => task.Complete(null!, outcome, output, completionBehaviorName);
        ...
        [Fact]
        public void Complete_With_Unauthorized_User_Should_Throw()
        {
            //arrange
            var task = HumanTaskFactory.Create();
            var user = task.PeopleAssignments.PotentialOwners!.First();
            ...
            task.Start(user);
            task.ClearPendingEvents();
            var action = () => task.Complete(UserReferenceFactory.Create(), outcome, output, completionBehaviorName);
        ...
        [Fact]
        public void Complete_With_Null_Outcome_Should_Throw()
        {
            //arrange
            var task = HumanTaskFactory.Create();
            var user = task.PeopleAssignments.PotentialOwners!.First();
            ...
            task.Start(user);
            task.ClearPendingEvents();
            var action = () => task.Complete(user, null!, output, completionBehaviorName);
        ...
        [Fact]
        public void Complete_When_Not_Active_Should_Throw()
        {
            //arrange
            var task = HumanTaskFactory.Create();
            var user = task.PeopleAssignments.PotentialOwners!.First();
            ...
            task.Complete(user, outcome, output, completionBehaviorName);
            task.ClearPendingEvents();
            var action = () => task.Complete(user, outcome, output, completionBehaviorName);
